        ...
        <div className="rounded-2xl bg-blue-200/10 shadow-xl p-8 space-y-5">
          <div className="text-center space-y-2">
            ...
            <p className=" text-muted-foreground">
              Please enter your credentials to <span className="uppercase text-blue-600">Login</span>
            </p>
          </div>
          {msg && <Alert title="Success" variant="success" message={msg} showLink={false} />}
          {error && (
            <Alert title="Invalid Credentials,Try again!" variant="error" message={errormsg} showLink={false} />
          )}
          {errorAlert && <span className="text-red-500 text-sm text-center">{errorAlert}</span>}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="e.g    johndoes@gmail.com" name="email" />
            </div>
          ...
          <div>
            ...
            <Link href="/signup" className="uppercase text-sm p-3 text-blue-400">
              Sign Up Here
            </Link>
          </div>
        </div>
