...
const CategoryNavItem = ({ categoryData, className }: Props) => {
  ...
  return (
    <li className={cn("relative px-4 py-3 cursor-pointer group", className)}>
      ...
      <Link
        href={"/list/" + group.url}
        className="text-gray-900 flex items-center font-medium transition-colors hover:text-red-600"
      >
        <span className="text-nowrap">{group.name}</span>
        <ChevronDown
          className="inline-block ml-1 text-gray-400 group-hover:text-gray-600 transition-colors"
          size={16}
        />
      </Link>
      ...
      {/* Dropdown */}
      {!!categories?.length && (
        <div className="absolute left-0 top-full mt-2 w-[700px] bg-white rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-20">
