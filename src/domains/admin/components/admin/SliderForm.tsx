...
export default function SliderForm({
  ...
  const handleImageDelete = (publicId: string) => async (e?: React.SyntheticEvent) => {
    e?.preventDefault();
    if (!publicId) return;
    try {
        ...
        <div className="flex items-center gap-3 flex-wrap mt-2">
          ...
          {imageValue && (
            <div className="relative w-28 h-28 rounded overflow-hidden border">
              ...
              <img src={JSON.parse(imageValue).url} alt="slider" className="w-full h-full object-cover" />
            </div>
          )}
        </div>
      </div>

      <input {...register("link")} placeholder="Product URL" className={inputClass} />

      <input {...register("alt")} placeholder="Alt text" className={inputClass} />

      <input {...register("title")} placeholder="Title" className={inputClass} />

      <input {...register("buttonText")} placeholder="Button text" className={inputClass} />
