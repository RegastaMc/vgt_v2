...
export const MobileCategoryAccordion = ({ categories, onClose }: Props) => {
  ...
  return (
    <div className=" min-h-screen absolute top-0 z-20 left-0 w-3/4 bg-gray-100 rounded-lg shadow-md">
      <div className="flex items-center justify-between p-4 border-b">
        <h2 className="text-lg bg-gray-100 text-black font-semibold">Categories</h2>
      ...
      <ul className="">
        {categories.map((group, gIndex) => (
          <li key={gIndex} className="p-4">
            {/* GROUP */}
            <button
              className="w-full flex justify-between font-semibold text-gray-900"
              onClick={() => setOpenGroup(openGroup === gIndex ? null : gIndex)}
            >
              {group.group.name}
              <span>{openGroup === gIndex ? "−" : "+"}</span>
            </button>
              ...
              <div className="mt-3 ml-3 space-y-2">
                {group.categories.map((cat, cIndex) => (
                  <div key={cIndex}>
                    ...
                    >
                      {cat.category.name}
                      {cat.subCategories?.length ? <span>{openCategory === cIndex ? "−" : "+"}</span> : null}
                    </button>
